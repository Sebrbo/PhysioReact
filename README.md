...
PhysioReact includes a web manifest and service worker for offline-capable use. The current service worker cache is `physioreact-v4`.
...
Le cache service worker courant est `physioreact-v4`. Les chemins versionnes principaux sont `css/style.css?v=4` et `js/app.js?v=3`, et ils doivent rester identiques dans `index.html` et `service-worker.js`.
